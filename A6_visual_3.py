...
for model in [1,2]:
    for layer in range(1,9):
        lr = 0.1
        weight_decay = 0.001
        print('Model %d, Layer %d' % (model,layer))
        for i in range(200):
            _, output = None, None
            if model == 1:
                _, output = model1(X,extract_features = layer)
                
            elif model == 2:
                _, output = model2(X,extract_features = layer)
            else:
                print('Error!')
                break
        ...
        plt.savefig('visualization/max_features_model%d_layer%d.png'%(model,layer), bbox_inches='tight')
        plt.close(fig)
        print('Model %d, Layer %d pic saved!' % (model,layer))